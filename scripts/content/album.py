...
class Album():
    ...
    def getAlbumID(self, url: str | None=None):
        if url:
            input_url = url
        elif self.url:
            input_url = self.url
    ...
    def getAlbum(self, albumID=None):
        ...
        response = requests.get(
            "https://www.jiosaavn.com/api.php?_format=json&__call=content.getAlbumDetails&albumid={0}".format(albumID), proxies=self.proxies, headers=self.headers)
